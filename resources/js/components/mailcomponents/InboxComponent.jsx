  ...
  render() {
    if(this.state.isWaiting) {
      ...
      if (this.state.showInboxes.length === 0) {
          ...
          <div>
            <div id="inboxSearch">
              <div className="py-2 w-11/12 mx-auto mt-1 relative">
                <input type="text" value={this.state.inboxSearch} id="inboxSearch" style={{ height:'45px',  }} className="w-full px-6 py-1 rounded-full bg-gray-100 border-none" onChange={this.handleOnChange} placeholder="Search here..."/>
        ...
        return (
          <div>
            <div id="deleteInboxConfirm" className="depositConfirm h-screen w-screen bg-black bg-opacity-70 fixed top-0 left-0 z-50 hidden">
              <div className="w-11/12 h-48 bg-white absolute rounded-xl" style={{ top:'50%', marginTop:'-6rem', left:'50%', marginLeft:'-45.83333%' }}>
                <div className="w-8/12 mx-auto h-20 mt-4">
                  <p className="text-center text-md md:text-lg text-gray-700 mt-5 mb-5">  Are you sure you want to delete this message? </p>
                </div>
                <div className="w-full h-16" id="confirmBtn">
                  <div className="w-full grid grid-cols-2 h-full">
                    <div className="col-span-1 h-full">
                      <button className="w-full h-full block mx-auto px-4 py-1 rounded-bl-lg text-gray-500  text-md md:text-lg bg-white" onClick={ () => $("div#deleteInboxConfirm").fadeOut(200) }>Cancel</button>
                    </div>
                    <div className="col-span-1">
                      <button className="w-full h-full block mx-auto px-4 py-1 rounded-br-lg text-white font-bold text-md md:text-lg" style={{ background:'#0ac2c8' }} onClick={ () => this.deleteInbox() }>Yes</button>
                    </div>
                  </div>
                </div>
              </div>
                    ...
                    return(
                      <div key={i} className="w-11/12 mx-auto rounded px-2 relative">
                          <div className="w-full">
                            <div className="w-full">
                              <img src={ constant.baseURL + 'storage/profile-image/' + inbox.accountInfo[0].avatar + '.jpg' } alt={ inbox.accountInfo[0].avatar} className="rounded-full" style={{ width:'55px', height:'55px', float:'left' }}/>
                                ...
                                <p style={{height:'20px', overflow:'hidden'}} className="text-gray-500 text-md relative">{inbox.inboxContent[0].content}
                                  {
                                    (inbox.unread)
                                      ?
                                      <span className="w-2 h-2 absolute rounded-full bg-red-500 bottom-1 right-1 block"/>
                                      :
                                      <span className="w-2 h-2 absolute rounded-full bg-red-500 bottom-1 right-1 hidden"/>
                                  }
                                </p>
                              </div>
                            </div>
                          </div>
                          <a onClick={() => this.showDeleteConfirmModal(inbox.id)}><i className="fas fa-times text-xs text-red-500 absolute bottom-1 right-0.5"/></a>
                        <hr className="pb-3"/>
                      </div>
                    );
                  })
